package utilities.cryption.huffman;
...
public class Huffman {
	...
	/**
	 * Represents the huffman file decryption keys.
	 */
	...
	/**
	 * Represents the huffman file masks.
	 */
	...
	/**
	 * Represents the huffman file bit sizes.
	 */
	...
	/**
	 * Decompresses huffman encoding.
	 * 
	 * @param buffer The {@code OutgoingPacket} buffer.
	 * @param length The length to use.
	 * @return the decoded string.
	 */
	public static String decompressHuffman(IoReadEvent buffer, int length) {
		byte[] textData = new byte[buffer.remaining()];
		buffer.readBytes(textData, buffer.remaining());
		return decompressViaHuffman(textData, length);
	}

	public static String decompressViaHuffman(byte[] message, int length) {
		try {
			int charsDecoded = 0;
			int keyIndex = 0;
	...
	public static void huffmanCompress(IoWriteEvent buffer, String message) {
		byte[] chatStr = new byte[256];
		chatStr[0] = (byte) message.length();
		byte length = (byte) (1 + huffmanCompress(message, chatStr, 1));
		buffer.writeBytes(chatStr, 0, length);
	}
